...
//   { part: 1, pallet_name: 1, variable: 1, sample_time: 1 },
...
//     await mongoose.connect("mongodb://127.0.0.1:27017/imr_sync");
//     // await mongoose.connect("mongodb://10.69.218.16/imr_sync");
...
// //       time_stamp: moment(record.sample_time)
// //       .tz("UTC")
// //       .format("YYYY-MM-DD HH:mm:ss"),
// //       serial_no : record.pallet_name,
// //       remarks: "",
// //     }));
...
// //         );
// //         logger.info(`✅ Successfully pushed ${pendingRecords.length} records to API.`);
// //       } else {
// //         logger.warn(
...
//     // ✅ Deduplicate by pallet_name + variable
//     const uniqueRecordsMap = new Map();
//     pendingRecords.forEach((rec) => {
//       const key = `${rec.pallet_name}_${rec.variable}`;
...
//       vc: record.variable, // ✅ API expects "vc"
//       data_point: record.data,
//       time_stamp: moment(record.sample_time).tz("UTC").format("YYYY-MM-DD HH:mm:ss"),
//       serial_no: record.pallet_name,
...
const API_URL = "http://localhost:8000/ax/demo/api/imrpost/";
const API_TOKEN = "a16be26101e5b2f83ccdb5f97ee6dd4e7525a3a3";
...
const sentRecordSchema = new mongoose.Schema(
  {
    ...
    data: { type: Number, required: true },
    sample_time: { type: Date, required: true },
    Pallet_No: { type: String, required: true },  // ✅ changed
    final_indicator: { type: String, required: true },
    equipment: { type: String, required: true },
    part: { type: String, required: true },
    sentToApi: { type: Boolean, default: false },
...
async function connectMongo() {
  if (mongoose.connection.readyState === 0) {
    await mongoose.connect("mongodb://localhost:27017/armature_line");
    logger.info("✅ Connected to MongoDB.");
  }
}
...
async function pushToCloud() {
  try {
    ...
    // ✅ Deduplicate by Pallet_No + variable
    const uniqueRecordsMap = new Map();
    pendingRecords.forEach((rec) => {
      const key = `${rec.Pallet_No}_${rec.variable}`;
      if (!uniqueRecordsMap.has(key)) {
