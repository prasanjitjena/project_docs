...
const { postData } = require('./mongo_test_diff10');
...
const csvFilePath = 'C:/zometric_diff/tpsolar_latest_aut/Diffusion10/converted.csv';
...
async function readCsv() {
  try {
    ...
    const lines = data.trim().split('\n');
    const headers = lines[0].trim().split(',');
    const req = ['Time', 'FurnaceID','TubeID', 'ZoneID', 'P1', 'P2', 'P3', 'P4', 'P5','P6','P7','P8','P9'];
    ...
    const rows = lines.slice(1).map(l => {
      ...
      idx.forEach((pos, i) => {
        ...
        if (req[i] === 'TubeID') {
          ...
          const m = v.match(/DIFFXBCL\d{2}-(\d+)/i);
          if (m) {
        ...
        if (['P1', 'P2', 'P3', 'P4', 'P5','P6','P7','P8','P9'].includes(req[i])) {
    ...
    // Skip row if any sample value (P1–P9) is 0.1 or 1
...
async function pollAllFolders() {
  console.log(`\n[${new Date().toISOString()}] Polling  Diffusion10 PC...`);
...
async function startPolling() {
  ...
  console.log(' Strategy: today → newest 2 TXT files | skip rows where P1-P9 = 0.1\n');
